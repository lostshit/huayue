 ...
 * @return mixed
 */
/*function generate_password($username,$len = 8)
{
	$len = $len >= 4 ? $len : 4;
	$ip = encrypt(real_ip());
	$ua = encrypt(get_user_browser());
	$password = substr(sha1(encrypt($username)),8,4). substr(sha1($ua . $ip .  microtime(TRUE) ) ,16,$len-4);
	$password = str_replace(array('1','l','0','o'),array('k','m','t','u'),$password);
	return $password;
}*/
...
{
    $len = $len >= 4 ? $len : 4;
    $password = mt_rand(pow(10,$len-1)+1,pow(10,$len)-1);
    return $password;
}
 ...
 */
function send_pwd_sms($sms_obj,$mobile, $uid, $user_name, $password)
{
	if (empty($uid) || empty($user_name) || empty($password) || empty($mobile))
	{
