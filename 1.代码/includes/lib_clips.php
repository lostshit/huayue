...
function get_user_default($user_id)
{
    ...
    $sql = "SELECT pay_points, user_money, credit_line, last_login, is_validated, is_mobile_validated FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '$user_id'";
    $row = $GLOBALS['db']->getRow($sql);
    $info = array();
    ...
    //手机号码验证
    $info['is_mobile_validated'] = !$row['is_mobile_validated']? 0:1;;
    /* 增加是否开启会员邮件验证开关 */
