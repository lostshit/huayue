...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        $sql = 'SELECT gg.img_url AS gg_img_url,gg.thumb_url AS gg_thumb_url,g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                "promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img, RAND() AS rnd " .
                'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
                "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
        "LEFT JOIN " . $GLOBALS['ecs']->table('goods_gallery') . " AS gg ".
        "ON gg.goods_id = g.goods_id AND gg.img_type = 'goods' ";
        ...
        file_put_contents('debug.log',var_export($result,true));
        foreach ($result AS $idx => $row)
        {
            ...
            $goods[$idx]['gg_thumb']        = get_image_path($row['goods_id'], $row['gg_thumb_url'], true);
            $goods[$idx]['goods_img']    = get_image_path($row['goods_id'], $row['goods_img']);
            $goods[$idx]['gg_img']    = get_image_path($row['goods_id'], $row['gg_img_url']);
