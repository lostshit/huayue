...
require_once(ROOT_PATH . 'includes/sms/ihacklog_sms.class.php');
$sms = ihacklog_sms::instance('72dns');
...
$not_login_arr =
array('sms_get_password','send_pwd_sms','instant_register','act_instant_register','login','act_login','register','act_register','act_edit_password','get_password','send_pwd_email','password', 'signin', 'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email','clear_history','qpassword_name', 'get_passwd_question', 'check_answer');

/* 显示页面的action列表 */
$ui_arr = array('sms_get_password','send_pwd_sms','instant_register','act_instant_register','register', 'login', 'profile', 'order_list', 'order_detail', 'address_list', 'collection_list',
...
/* 传说中的1秒注册 */
elseif ($action == 'instant_register')
{
	if ((!isset($back_act)||empty($back_act)) && isset($GLOBALS['_SERVER']['HTTP_REFERER']))
	{
		$back_act = strpos($GLOBALS['_SERVER']['HTTP_REFERER'], 'user.php') ? './index.php' : $GLOBALS['_SERVER']['HTTP_REFERER'];
	...
	/* 取出注册扩展字段,手机号 id 为 5*/
	$sql = 'SELECT * FROM ' . $ecs->table('reg_fields') . ' WHERE id=5 LIMIT 1';
	$extend_info_list = $db->getAll($sql);
	$smarty->assign('extend_info_list', $extend_info_list);
	...
	$smarty->assign('shop_reg_closed', $_CFG['shop_reg_closed']);
	//    $smarty->assign('back_act', $back_act);
	$smarty->display('instant_user_passport.dwt');
}


/* 1秒注册会员的处理 */
elseif ($action == 'act_instant_register')
{
	/* 增加是否关闭注册 */
	...
	else
	{
		...
		$password = generate_password($username,6);
		$other['msn'] = '';
		$other['qq'] = '';
		$other['office_phone'] = '';
		$other['home_phone'] =  '';
		...
		//根据手机号自动选择邮箱
		function mobile2email($mobile_num)
		{
			$email = $mobile_num . '@allweb.com.cn';
			//中国移动号段
			/*
			$cmcc_arr = array(
			...
			*/
			if(preg_match("/^0?1((3[4-9]|47|5[0-27-9]|8[2-47-8]))\d{8}$/",$mobile_num))
			{
				$mobile_num = $mobile_num . '';
				$email = sprintf('%s@139.com',$mobile_num);
				return $email;
			}

			if(preg_match("/^0?1((3[0-2]|45|5[5-6]|8[5-6]))\d{8}$/",$mobile_num))
			{
				$mobile_num = $mobile_num . '';
				$email = sprintf('%s@wo.com.cn',$mobile_num);
				return $email;
			}

			if(preg_match("/^0?1((33|47|53|8[0-19]))\d{8}$/",$mobile_num))
			{
				$mobile_num = $mobile_num . '';
				$email = sprintf('%s@189.cn',$mobile_num);
				return $email;
			}
			//没检测出来，
			return $email;
		...
		if (strlen($other['mobile_phone']) < 11 || !preg_match("/^0?1((3|8)[0-9]|5[0-35-9]|4[57])\d{8}$/", $other['mobile_phone']))
		{
			show_message($_LANG['passport_js']['mobile_phone_invalid']);
		}
		
		//转换手机号为email
		$email    = mobile2email($other['mobile_phone']);
		/* 验证码检查 */
		if ((intval($_CFG['captcha']) & CAPTCHA_REGISTER) && gd_version() > 0)
		{
		...
		if (register($username, $password, $email, $other) !== false)
		{
			...
			/* 自动发送密码信息到用户手机 */
			$sms_send_rs = send_pwd_sms($sms,$other['mobile_phone'], $_SESSION['user_id'], $username, $password);
			file_put_contents('debug.log', var_export($sms->get_last_error(),TRUE),FILE_APPEND);
			if( FALSE === $sms_send_rs )
			{
				$auto_redirect = FALSE;
				$sms_status = sprintf('系统错误： 发送短信失败！<a href="%s">点击这里重新发送</a>','user.php?act=sms_reset_pwd');
			}
			else
			{
				$auto_redirect = TRUE;
				$sms_status = '密码已经通过短信成功发送到您的手机。';
			}
			/* 判断是否需要自动发送注册邮件 */
			...
			//show_message(sprintf($_LANG['register_success'] . '<br />'. $sms_status, $username . $ucdata), array($_LANG['back_up_page'], $_LANG['profile_lnk']), array($back_act, 'user.php'), 'info', $auto_redirect);
			echo  $username;
		}
		else
		{
...
//已登录的情况下重新通过短信发送密码
elseif($action == 'sms_reset_pwd')
{
	include_once(ROOT_PATH . 'includes/lib_passport.php');

	$sql = "SELECT user_name, mobile_phone FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '$user_id'";
	$row = $GLOBALS['db']->getRow($sql);
	$password = generate_password($row['user_name'],6);
	$sms_send_rs = send_pwd_sms($sms,$row['mobile_phone'], $_SESSION['user_id'], $row['user_name'], $password);
	file_put_contents('debug.log', var_export($sms->get_last_error(),TRUE),FILE_APPEND);
	if( FALSE === $sms_send_rs )
	{
		$auto_redirect = FALSE;
	...
	else
	{
		...
		$sms_status = '密码已经通过短信成功发送到您的手机。';
		$users =& init_users();
	    if ($users->edit_user(array('username'=>$row['user_name'], 'password'=>$password), 1))
        {
	...
	show_message($_LANG['edit_password_success'] . '<br />'. $sms_status, array($_LANG['back_up_page'], $_LANG['profile_lnk']), array($back_act, 'user.php'), 'info', $auto_redirect);
...
elseif ($action == 'send_pwd_sms')
{
	...
	$user_mobile = $GLOBALS['db']->getOne("SELECT mobile_phone FROM ". $GLOBALS['ecs']->table('users'). " WHERE user_id='$user_id' LIMIT 1");
	if ($user_info && $user_info['user_name'] == $user_name && $mobile_phone == $user_mobile)
	{
		//生成新的密码
		$new_password = generate_password($user_info['user_name'], 6);

		//发送邮件的函数
		if (send_pwd_sms($sms,$user_mobile,$user_info['user_id'], $user_name, $new_password))
		{
			$users =& init_users();
			//发送成功，更新密码
			if( $users->edit_user(array('username'=> $user_info['user_name'], 'password'=>$new_password), 1) )
            {
                //直接把ec_salt置0
                $sql="UPDATE ".$ecs->table('users'). "SET `ec_salt`='0' WHERE user_id= '".$user_id."'";
                $db->query($sql);
			...
			unset($users);
			show_message($_LANG['send_success_sms'] . $user_mobile, $_LANG['back_home_lnk'], './', 'info');
		}
		else
		{
			//发送短信出错
			show_message($_LANG['fail_send_password_sms'], $_LANG['back_page_up'], './', 'info');
		}
	}
	else
	{
...
elseif ($action == 'act_edit_password')
{
    ...
    if (($user_info && (!empty($code) && md5($user_info['user_id'] . $_CFG['hash_code'] . $user_info['reg_time']) == $code)) || ($_SESSION['user_id']>0 && $_SESSION['user_id'] == $user_id && $user->check_user($_SESSION['user_name'], $old_password)))
    {
        ...
        if ($user->edit_user(array('username'=> (empty($code) ? $_SESSION['user_name'] : $user_info['user_name']), 'old_password'=>$old_password, 'password'=>$new_password), empty($code) ? 0 : 1))
        {
			$sql="UPDATE ".$ecs->table('users'). "SET `ec_salt`='0' WHERE user_id= '".$user_id."'";
			$db->query($sql);
			//修改密码就证明用户已经从短信收到了密码，把手机验证设置为已验证
