...
function set_flash_data($tplname, &$msg)
{
    ...
    switch($tplname)
    {
        ...
        case 'dynfocus':
            ...
            break;
        case 'carousel':
            $msg = set_img_carousel($tplname, $flashdata);
            break;
        case 'default':
        default:
 ...
 * @return bool
 */
function set_img_carousel($tplname, $flashdata)
{
    $data_file = ROOT_PATH . DATA_DIR . '/flashdata/' . $tplname . '/data.js';
    $jsdata = 'var carousel_html="';
    foreach ($flashdata as $data)
    {
        $bg = isset($data['bg_src']) ? 'background:url('. $data['bg_src'] .');': 'background:#000;';
        $jsdata .= sprintf('<li style=\\"%s\\"><img src=\\"%s\\" alt=\\"%s\\"/></li>',$bg,$data['src'],$data['url'],$data['desc']);
    }
    $jsdata .= '";';
    file_put_contents($data_file, $jsdata);
    return true;
