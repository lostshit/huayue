...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    else
    {
        ...
        if (empty($goods) === true)
        {
            ...
            $goods = array(
                ...
                'warn_number'   => 1,
                'promote_start_date' => local_date('Y-m-d'),
                'promote_end_date'   => local_date('Y-m-d', gmstr2time('+1 month')),
                'goods_weight'  => 0,
                'give_integral' => -1,
                'rank_integral' => -1,
                //增加7个新的字段
        ...
        /* 图片列表 */
        $sql = "SELECT * FROM " . $ecs->table('goods_gallery') . " WHERE goods_id = '$goods[goods_id]' AND img_type='goods'";
        $img_list = $db->getAll($sql);
        /* 全景图片 */
        $sql = "SELECT * FROM " . $ecs->table('goods_gallery') . " WHERE goods_id = '$goods[goods_id]' AND img_type='panorama'";
        $panorama_img_list = $db->getAll($sql);
        ...
        if (isset($GLOBALS['shop_id']) && ($GLOBALS['shop_id'] > 0))
        {
            ...
            foreach ($panorama_img_list as $key => $panorama_gallery_img)
            {
            	$panorama_gallery_img[$key]['img_url'] = get_image_path($panorama_gallery_img['goods_id'], $panorama_gallery_img['img_original'], false, 'gallery');
            	$panorama_gallery_img[$key]['thumb_url'] = get_image_path($panorama_gallery_img['goods_id'], $panorama_gallery_img['img_original'], true, 'gallery');
            }
        }
        else
        {
            ...
            foreach ($panorama_img_list as $key => $panorama_gallery_img)
            {
            	$panorama_gallery_img[$key]['thumb_url'] = '../' . (empty($panorama_gallery_img['thumb_url']) ? $panorama_gallery_img['img_url'] : $panorama_gallery_img['thumb_url']);
            }
        }
    }
    ...
    $smarty->assign('panorama_img_list', $panorama_img_list);
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    //增加7个新的字段
    $house_unit = isset($_POST['house_unit']) ? trim($_POST['house_unit']) : '一';
    $house_floor = isset($_POST['house_floor']) ? trim($_POST['house_floor']) : 23;
    $house_room = isset($_POST['house_room']) ? trim($_POST['house_room']) : 02;
    $house_status = isset($_POST['house_status']) ? trim($_POST['house_status']) : 'onsale';
    $house_type = isset($_POST['house_type']) ? trim($_POST['house_type']) : '未知';
    $house_floorage = isset($_POST['house_floorage']) ? trim($_POST['house_floorage']) : 100.0;
    $house_useage = isset($_POST['house_useage']) ? trim($_POST['house_useage']) : 90.33;
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                ...
                "house_unit, house_room, house_status, house_type, house_floor, house_floorage, house_useage )" .
    ...
    else
    {
        ...
        $sql = "UPDATE " . $ecs->table('goods') . " SET " .
                ...
                "house_floorage = '$house_floorage', " .
                "house_useage = '$house_useage', " .
    ...
    /* 处理全景图 */
    handle_gallery_image($goods_id,  $_FILES['panorama_img_url'], $_POST['panorama_img_desc'], $_POST['panorama_img_file'],'panorama','panorama');
