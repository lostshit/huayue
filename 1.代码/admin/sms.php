...
switch ($action)
{
   ...
   case 'register' :
       ...
       $result = $sms->register($username, $password, $api_url, $phone);

       $link[] = array('text'  =>  $_LANG['back'],
                       'href'  =>  'sms.php?act=display_my_info');

       $balance = $sms->get_balance();
       if ($balance && ($balance > 0) )//注册成功
       {
           sys_msg($_LANG['register_ok'], 0, $link);
       }
       else
       {
       		$err_arr =array_pop($sms->get_errors());
           $error_detail = '错误代码：'. $err_arr['code']. '  错误信息：'. $err_arr['msg'];
...
//        break;
    ...
    case 'update_balance':
    	$balance = $sms->get_balance();
    	$sms->update_config('balance',$balance);
    	$link[] = array('text'  =>  '点击这里返回',
    			'href'  =>  'sms.php?act=display_my_info');
    	sys_msg('更新成功.', 0, $link);
    	break;
    /* 发送短信 */
...
//
   ...
   default :
       /* 检查权限 */
        ...
        //test
        $link[] = array('text'  =>  $_LANG['back'],
        		'href'  =>  'sms.php?act=api_config');

        $balance = $sms->get_balance();
        if( is_numeric($balance) && (FALSE !== $balance))
        {
	        if ( $balance == 0 )
	        {
        	    $err_arr =array_pop($sms->get_errors());
	        	$error_detail = '错误：可用短信余数为0！';
	        	sys_msg($_LANG['register_error'] . $error_detail, 1, $link);
	        }
        }
        else
        {
           ...
           $sms_my_info = $sms->get_site_info();//这里不再进行判空处理，主要是因为如果前个式子不出错，这里一般不会出错
// 			var_dump($sms_my_info);
           /* 格式化时间输出 */
