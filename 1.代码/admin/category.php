...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('action_link2',  array('href' => 'category.php?act=add&parent_id=0', 'text' => '添加项目组团'));
    $smarty->assign('action_link',  array('href' => 'category.php?act=add', 'text' => '添加栋号'));
...
if (isset($_FILES['group_airscape']['error'])) // php 4.2 版本才支持 error
{
	// 最大上传文件大小
	...
	// 商品图片
	if ($_FILES['group_airscape']['error'] == 0)
	{
		if (!$image->check_img_type($_FILES['group_airscape']['type']))
		{
			sys_msg($_LANG['invalid_goods_img'], 1, array(), false);
		}
	}
	elseif ($_FILES['group_airscape']['error'] == 1)
	{
		sys_msg(sprintf($_LANG['goods_img_too_big'], $php_maxsize), 1, array(), false);
	}
	elseif ($_FILES['group_airscape']['error'] == 2)
	{
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    /* 初始化分类属性 */
    $cat_attr['group_airscape'] = !empty($_POST['group_airscape']) ? $_POST['group_airscape'] : '';
    $cat_attr['group_price'] = !empty($_POST['group_price']) ? $_POST['group_price'] : '';
    $cat_attr['group_age'] = !empty($_POST['group_age']) ? $_POST['group_age'] : '';
    $cat_attr['group_property'] = !empty($_POST['group_property']) ? $_POST['group_property'] : '';
    $cat_attr['group_cell'] = !empty($_POST['group_cell']) ? $_POST['group_cell'] : '';

    $cat_attr['build_num'] = !empty($_POST['build_num']) ? $_POST['build_num'] : '';
    $cat_attr['build_coordinates'] = !empty($_POST['build_coordinates']) ? $_POST['build_coordinates'] : '';
    $cat_attr['build_price'] = !empty($_POST['build_price']) ? $_POST['build_price'] : '';
    $cat_attr['build_permit'] = !empty($_POST['build_permit']) ? $_POST['build_permit'] : '';
    $cat_attr['build_open'] = !empty($_POST['build_open']) ? $_POST['build_open'] : '';
    $cat_attr['build_enter'] = !empty($_POST['build_enter']) ? $_POST['build_enter'] : '';
    $cat_attr['build_floor'] = !empty($_POST['build_floor']) ? $_POST['build_floor'] : '';
    $cat_attr['build_unit'] = !empty($_POST['build_unit']) ? $_POST['build_unit'] : '';
    $cat_attr['build_cell'] = !empty($_POST['build_cell']) ? $_POST['build_cell'] : '';
    ...
    if ($db->autoExecute($ecs->table('category'), $cat) !== false)
    {
		...
		//先插入一次
        $cat_attr['cat_id'] = $cat_id;
        $db->autoExecute($ecs->table('cat_attribute'), $cat_attr);
        //图片处理
        insert_or_udpate_cat_picture();
...
if ($_REQUEST['act'] == 'update')
{
    ...
    $db->autoExecute($ecs->table('cat_attribute'), $cat_attr, 'UPDATE', "cat_id='$cat_id'");
 ...
 * 插入或更新图片时处理
 */
function insert_or_udpate_cat_picture()
{
	global $db, $ecs, $_CFG, $image, $cat_id, $cat_attr;
...
if( $_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
	...
	// 如果上传了商品图片，相应处理
	if (($_FILES['group_airscape']['tmp_name'] != '' && $_FILES['group_airscape']['tmp_name'] != 'none') or (($_POST['group_airscape_url'] != $_LANG['lab_picture_url'] && $_POST['group_airscape_url'] != 'http://') && $is_url_goods_img = 1))
	{
		//更新
		if ($_REQUEST['cat_id'] > 0)
		{
			/* 删除原来的图片文件 */
			$sql = "SELECT group_airscape " .
					" FROM " . $ecs->table('cat_attribute') .
					" WHERE cat_id = '$_REQUEST[cat_id]'";
			$row = $db->getRow($sql);
			if ($row['group_airscape'] != '' && is_file('../' . $row['group_airscape']))
			{
				@unlink('../' . $row['group_airscape']);
			}
		}
		...
		{
			$original_img   = $image->upload_image($_FILES['group_airscape']); // 原始图片
		}
		elseif (copy(trim($_POST['group_airscape_url']), ROOT_PATH . 'temp/' . basename($_POST['group_airscape_url'])))
		{
			$original_img = 'temp/' . basename($_POST['group_airscape_url']);
		}
		...
		if ($proc_thumb && $image->gd_version() > 0 && $image->check_img_function($_FILES['group_airscape']['type']) || $is_url_goods_img)
		{
		...
		/* 重新格式化图片名称 */
		$group_airscape_img = reformat_cat_image_name('group_airscape', $cat_id, $original_img, 'group_airscape');
		$cat_attr['group_airscape'] = $group_airscape_img;
		//更新图片数据
		$db->autoExecute($ecs->table('cat_attribute'), $cat_attr,'UPDATE', "cat_id='$cat_id'");
	}
}
...
if ($_REQUEST['act'] == 'remove')
{
    ...
    /* 分类图片 */
    $group_airscape = $db->getOne('SELECT group_airscape FROM ' .$ecs->table('cat_attribute'). " WHERE cat_id='$cat_id'");
    ...
    if ($cat_count == 0 && $goods_count == 0)
    {
        ...
        {
        	/* 删除图片group_airscape */
        	if (!empty($group_airscape))
        	{
        		@unlink('../' . $group_airscape);
        	}
        	//删除cat_attribute记录
            $db->query("DELETE FROM " . $ecs->table('cat_attribute') . " WHERE cat_id = '" . $cat_id ."'");
...
function get_cat_info($cat_id)
{
    ...
    " AS c LEFT JOIN " .$GLOBALS['ecs']->table('cat_attribute'). " AS ca ON c.cat_id = ca.cat_id WHERE c.cat_id='$cat_id' LIMIT 1";
 ...
 * $position 用于决定目录
 */
function reformat_cat_image_name($type, $cat_id, $source_img, $position='')
{
	...
	//添加全景图  by 荒野无灯  2013-05-13
	if (!make_dir(ROOT_PATH.$dir.'/'.$sub_dir.'/group_airscape_img'))
	{
		return false;
	}
	...
	switch($type)
	{
			//添加全景图  by 荒野无灯  2013-05-13
		case 'group_airscape':
			$img_name = $cat_id . '_A_' . $rand_name;
			break;
	}

	if ($position == 'group_airscape')
	{
		if (move_image_file(ROOT_PATH.$source_img, ROOT_PATH.$dir.'/'.$sub_dir.'/group_airscape_img/'.$img_name.$img_ext))
		{
			return $dir.'/'.$sub_dir.'/group_airscape_img/'.$img_name.$img_ext;
		}
	}
	return false;
}
