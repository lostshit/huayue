 ...
 * @param   string  $img_type ( goods or panorama )
 * @return  void
 */
function handle_gallery_image($goods_id, $image_files, $image_descs, $image_urls,$img_position = 'goods',$img_type='goods')
{
	if( !in_array($img_type, array('goods','panorama')) )
	{
		$img_type = 'panorama';
	}
    /* 是否处理缩略图 */
    ...
    foreach ($image_descs AS $key => $img_desc)
    {
        ...
        if ($flag)
        {
            ...
            $img_url = reformat_image_name($img_type, $goods_id, $img_url, $img_position );
            $thumb_url = reformat_image_name('gallery_thumb', $goods_id, $thumb_url, 'thumb');
            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('goods_gallery') . " (goods_id, img_url, img_desc, thumb_url, img_original,img_type) " .
                    "VALUES ('$goods_id', '$img_url', '$img_desc', '$thumb_url', '$img_original', '$img_type')";
            $GLOBALS['db']->query($sql);
            /* 不保留商品原图的时候删除原图 */
...
function reformat_image_name($type, $goods_id, $source_img, $position='')
{
    ...
    if (!make_dir(ROOT_PATH.$dir.'/'.$sub_dir.'/panorama_img'))
    {
    	return false;
    }
    ...
    switch($type)
    {
        ...
        //添加全景图  by 荒野无灯  2013-05-13
		case 'panorama':
           	$img_name = $goods_id . '_R_' . $rand_name;
           	break;
    }
    ...
    //添加全景图  by 荒野无灯  2013-05-13
    elseif  ($position == 'panorama')
    {
    	if (move_image_file(ROOT_PATH.$source_img, ROOT_PATH.$dir.'/'.$sub_dir.'/panorama_img/'.$img_name.$img_ext))
    	{
    		return $dir.'/'.$sub_dir.'/panorama_img/'.$img_name.$img_ext;
    	}
    }
    else
    {
